...
def read_member_csv(path):
    ...
    의원-당적 CSV를 읽는다.
    지정된 열 구조를 그대로 사용한다:
      3열 의원명 / 4열 정당 / 5열 지역 / 6열 성별 / 7열 당선횟수
    """
    ...
    for row_index, row in enumerate(rows):
        ...
        # 첫 행에 헤더가 있는 경우 건너뜀
        if row_index == 0:
            header = [normalize_name(x) for x in row[:7]]
            if "의원명" in header or "정당" in header:
                continue
        ...
        mapping[name] = {
            "party": str(row[3]).strip(),
            "region": str(row[4]).strip(),
            "gender": str(row[5]).strip(),
            "election_count": str(row[6]).strip(),
        }
