...
speaker_speech_detail = (
    ...
    .with_columns(pl.col("Date_Parsed").dt.strftime("%Y-%m-%d").alias("Date"))
    .select([
        "Assembly_Num", "Title", "Date", "Date_Parsed", "Speaker", "Title_Role",
        "Speaker_Source", "Speaker_Type", "Party", "Committee", "Region",
        "Speech", *KEYWORDS_EN, "Keyword_Total", "Keyword_Spaced_Extra",
        "Discourse_Development", "Discourse_Boundary",
    ])
    .sort(["Date_Parsed", "Title"])
    .drop("Date_Parsed")
)
speaker_speech_detail.write_csv(f"{OUTPUT_DIR}/speaker_speech_detail.csv")

# 검수 대상: 이름을 못 찾았거나(none) 규칙으로 추정한(fallback) 행만
review = speaker_speech_detail.filter(
    pl.col("Speaker_Source").is_in(["none", "fallback"])
)
review.write_csv(f"{OUTPUT_DIR}/unmatched_speaker_review.csv")
...
if learned_rows:
    pl.DataFrame(
        ...
        orient="row",
    ).write_csv(f"{OUTPUT_DIR}/learned_officer_names.csv")
...
speaker_keyword_summary.write_csv(f"{OUTPUT_DIR}/speaker_keyword_summary.csv")
...
party_keyword_summary.write_csv(f"{OUTPUT_DIR}/party_keyword_summary.csv")
