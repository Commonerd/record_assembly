"""
국회 회의록 발언자 파싱 · 키워드 집계 · 네트워크 구축 (v4)
==============================================================================
v4 변경 요약 (v3 대비)
------------------------------------------------------------------------------
[파서 보강] speaker_speech_detail.csv에 "직책 이름" 헤더가 본문에 남는
문제의 원인 4가지를 수정:
  1) [이름]+[직책] 순서의 미등재 이름 — v3의 fallback은 [직책]+[이름]
     순서만 커버해서, 증인·참고인·정부위원처럼 의원 명부에 없는 이름이
     앞에 오면 헤더 전체가 본문에 남았다. 역방향 fallback(B) 추가.
  2) TITLE_CORE 누락 직책 — 처장·차장·원장·총재·사령관·지사·시장·군계급 추가.
  3) 학습 후보 수집기가 '직책+경계'가 파싱된 행만 수집 — 직책과 이름이
     붙은 경우("공보처장김활란…")도 직책 접두를 떼어 수집하도록 확대.
  4) 학습 임계값 min_count 5 -> 3 (발언 3~4회 직책자도 학습).
  + Residual_Header_Flag: 파싱 후에도 본문이 헤더처럼 시작하는 행을 표시,
    검수 파일에 포함. 반복 직책은 TITLE_CORE로, 이름은 officer_names.csv로
    환류하는 사람-루프를 만든다.

[네트워크 신규] 피드백("텍스트마이닝에 물방울 몇 개를 더하는 것을 넘어
볼거리가 되는 네트워크") 및 참조 논문 2편의 조작화를 반영:
  - 발언 인접 네트워크 (Bingenheimer 2021의 nexus point를 회의록에 번역:
    같은 회의에서 연이어 발언 = 기록된 공동 출현. 관계를 '추정'하지 않음)
  - 회기별 정당 입장 네트워크 (Card et al. 2022의 정당별 frame 궤적을
    노드·간선으로: 회기 내 정당 간 키워드 프로필 코사인 유사도)
  - Gephi/Cytoscape용 GEXF + 자체완결 HTML 타임라인
    (assembly_history_overview.html — Bingenheimer Fig.1의 국회판)
자세한 조작화 근거는 §7-5 주석 참고.
...
TITLE_CORE = (
    ...
    r"|[가-힣]{0,8}차관"
    r"|[가-힣]{0,10}처장"            # 공보처장, 법제처장 …  (v4 추가)
    r"|[가-힣]{0,10}차장"            # (v4 추가)
    r"|[가-힣]{0,10}부장"
    r"|[가-힣]{0,10}국장"            # 육군본부인사국장 …
    r"|[가-힣]{0,10}청장"
    r"|[가-힣]{0,10}실장"
    r"|[가-힣]{0,10}총장"            # 사무총장, 검찰총장 …
    r"|[가-힣]{0,10}원장"            # 심계원장, 감찰위원회원장 …  (v4 추가)
    r"|[가-힣]{1,10}총재"            # 한국은행총재 …  (v4 추가)
    r"|[가-힣]{2,10}사령관"          # 계엄사령관 …  (v4 추가)
    r"|[가-힣]{1,10}지사"            # 도지사 (v4 추가; 단독 '지사'는 오탐 위험이라 접두어 필수)
    r"|[가-힣]{2,10}시장"            # 서울특별시장 (v4 추가; 단독 '시장'은 market과 충돌하므로 접두어 필수)
    r"|[가-힣]{2,10}(?:준장|중장|대령)"  # 군 계급 (v4 추가; '소장'은 訴狀과 충돌하므로 제외)
    r"|임시의장|부의장|의장"
    r"|국무총리|국무위원|정부위원|전문위원"
    r"|위원|의원|증인|참고인|서기관|서기)"
)
...
def apply_fallback(lazy: pl.LazyFrame) -> pl.LazyFrame:
    """사전 매칭 실패분에 대한 보수적 규칙 추정 (모두 검수 대상 플래그).

    (A) [직책]은 잡혔는데 이름이 사전에 없는 경우:
        Speech 맨 앞의 2~4글자를 이름 후보로 추정.  예) "증인 김활란 저는…"
    (B) [이름]+[직책] 순서인데 이름이 사전에 없는 경우 (v4 추가):
        v3에서는 이 순서의 미등재 이름을 전혀 잡지 못해 헤더 전체가 본문에
        남았다 — 검수 파일에서 확인된 잔존 헤더의 최대 원인.
        예) "김활란증인 저는…", "박순천 의원 지금…"
        직책이 바로 뒤따를 때만 동작하므로 본문 오인 위험이 낮다.
    """
    # --- (A) [직책] + [미등재 이름] ---
    need_a = (pl.col("Title_Role") != "") & (pl.col("Speaker") == "")
    cand_a = pl.col("Speech").str.extract(FALLBACK_NAME_RE, 1)
    ok_a = need_a & cand_a.is_not_null() & ~cand_a.is_in(list(NAME_STOPWORDS))
    lazy = lazy.with_columns([
        pl.when(ok_a).then(cand_a).otherwise(pl.col("Speaker")).alias("Speaker"),
        pl.when(ok_a)
        .then(pl.col("Speech").str.replace(FALLBACK_NAME_RE, "").str.strip_chars())
        .otherwise(pl.col("Speech"))
        .alias("Speech"),
        ok_a.alias("_fallback_used"),
    ])

    # --- (B) [미등재 이름] + [직책] ---
    NAME_TITLE_RE = (
        r"^(" + GENERIC_NAME + r")" + PAREN + r"[\s,·]*(" + TITLE_PAT + r")"
        + PAREN + BOUND
    )
    need_b = (pl.col("Title_Role") == "") & (pl.col("Speaker") == "")
    cand_name = pl.col("Speech").str.extract(NAME_TITLE_RE, 1)
    cand_role = pl.col("Speech").str.extract(NAME_TITLE_RE, 2)
    ok_b = need_b & cand_name.is_not_null() & ~cand_name.is_in(list(NAME_STOPWORDS))
    return lazy.with_columns([
        pl.when(ok_b).then(cand_name).otherwise(pl.col("Speaker")).alias("Speaker"),
        pl.when(ok_b).then(cand_role).otherwise(pl.col("Title_Role")).alias("Title_Role"),
        pl.when(ok_b)
        .then(pl.col("Speech").str.replace(NAME_TITLE_RE, "").str.strip_chars())
        .otherwise(pl.col("Speech"))
        .alias("Speech"),
        (pl.col("_fallback_used") | ok_b).alias("_fallback_used"),
    ])


def flag_residual_headers(lazy: pl.LazyFrame) -> pl.LazyFrame:
    """파싱이 모두 끝난 뒤에도 Speech가 여전히 헤더처럼 시작하는 행을 표시 (v4).

    이 플래그가 true인 행은 (a) TITLE_CORE에 아직 없는 직책이거나
    (b) 사전·학습·fallback 모두 놓친 이름이다. 검수 파일에 포함시켜
    사람이 훑어보고 officer_names.csv / TITLE_CORE 보강으로 환류시킨다.
    """
    residual = (
        pl.col("Speech").str.contains(r"^" + TITLE_PAT + r"(?:[\s,.·(]|$)")
        | pl.col("Speech").str.contains(
            r"^[가-힣]{2,4}\s*" + TITLE_PAT + r"(?:[\s,.·(]|$)"
        )
    )
    return lazy.with_columns(residual.alias("Residual_Header_Flag"))
...
def learn_officer_names(residual_counts: Counter, min_count: int = 3,
...
def collect_officer_candidates(lazy: pl.LazyFrame) -> dict:
    """Pass 1 결과에서 이름 미상 행의 잔여 문자열을 모은다.

    v4 변경: Title_Role이 경계까지 깔끔하게 파싱된 행만 보던 것을,
    Speech가 직책 패턴으로 '시작하기만' 하면 수집하도록 넓혔다.
    "공보처장김활란지금…"처럼 직책과 이름이 붙은 경우도 직책 접두를
    떼어낸 잔여("김활란지금…")가 학습기로 들어간다.
    """
    C = pl.col("Speech")
    title_prefix = C.str.extract(r"^(" + TITLE_PAT + r")", 1)
    resid = (
        lazy.filter(pl.col("Speaker") == "")
        .with_columns([
            # 이미 파싱된 직책이 있으면 그것을, 없으면 Speech 맨 앞에서 추출
            pl.when(pl.col("Title_Role") != "")
            .then(pl.col("Title_Role"))
            .otherwise(title_prefix.fill_null(""))
            .alias("_role"),
            pl.when(pl.col("Title_Role") != "")
            .then(C)
            .otherwise(C.str.replace(r"^" + TITLE_PAT, ""))
            .str.replace_all(r"\s+", "")
            .str.slice(0, 5)
            .alias("Residual"),
        ])
        .filter(pl.col("_role") != "")
        .group_by(["Assembly_Num", "_role", "Residual"])
        .agg(pl.len().alias("n"))
    )
    df = _collect(resid)
    ...
    for row in df.iter_rows(named=True):
        if row["Residual"]:
            buckets[(row["Assembly_Num"], row["_role"])][row["Residual"]] += row["n"]
    return buckets
...
SELFTEST_CASES = [
    ...
    # --- v4 신규 케이스: 사전에 없는 이름 / 새 직책 ---
    ("박순천 의원 지금 말씀드리면", "의원", "박순천"),        # 미등재 이름+직책 (fallback B)
    ("증인 김활란 저는 그날", "증인", "김활란"),              # 직책+미등재 이름 (fallback A)
    ("공보처장 김활란 보고드립니다", "공보처장", "김활란"),    # v4 신규 직책(처장)
    ("한국은행총재 김유택 금융 사정을", "한국은행총재", "김유택"),
    ("계엄사령관 이준식 계엄 상황은", "계엄사령관", "이준식"),
    ("심계원장 명제세 심계 결과를", "심계원장", "명제세"),
]
...
def run_selftest() -> int:
    ...
    got = apply_fallback(parse_header(df.lazy(), alt)).collect()
    fails = 0
...
final_lazy = flag_residual_headers(final_lazy)
...
review = detail.filter(
    pl.col("Speaker_Source").is_in(["none", "fallback"])
    | pl.col("Residual_Header_Flag")
)
...
# ==============================================================================
# 7-5. (v4 신규) 네트워크 산출물 — "국회의 역사를 한 눈에"
# ==============================================================================
# 참조한 두 논문의 조작화(operationalization)를 국회 회의록에 맞게 번역했다.
#
# [Bingenheimer 2021, 中國佛敎 역사 네트워크]
#   그 논문의 데이터 단위는 "nexus point": 두 인물이 같은 시간·같은 장소에
#   있었다는 기록 자체를 연결로 본다 (무슨 일이 있었는지는 기록하지 않음).
#   국회 회의록의 등가물: "같은 회의에서 발언했다"가 곧 같은 시간·장소의
#   공동 출현이다. 다만 본회의는 수백 명이 출석하므로 전체 조합(n²) 연결은
#   무의미하다. 대신 **발언 인접(adjacency)**을 쓴다 — 회의에서 연이어
#   발언한 두 사람은 실제로 응답·질의 관계일 확률이 높다 (장관 답변,
#   의사진행 발언 등). 피드백에서 지적된 "회의록만으로 인물 관계 재구성은
#   힘들다"에 대한 보수적 대안: 관계를 '주장'하지 않고 발언 순서라는
#   기록된 사실만 연결로 옮긴다.
#
# [Card et al. 2022, PNAS 140년 이민 담론]
#   정당을 시기별로 갈라 담론 지표(frame 사용률)의 궤적을 비교한다.
#   국회 등가물: (대수, 회기, 정당) 노드에 키워드 사용률 벡터를 달고,
#   같은 회기 내 정당 쌍의 벡터 유사도를 연결로 삼는다 = "회기별 정당
#   입장 네트워크". 정당 노드를 회기 순서로 이어붙이면(시간 등뼈)
#   Bingenheimer Fig.1 같은 시간축 띠 구조가 나온다.
#
# 산출물:
#   speaker_interaction_nodes/edges.csv + .gexf   (Gephi/Cytoscape용)
#   party_stance_nodes/edges.csv + .gexf
#   assembly_history_overview.html               (자체완결 SVG 타임라인)
#
# 참고: "안건별" 네트워크는 회의록 Content 안의 안건 구분("1. 보고사항" 등)
# 세그먼트화가 선행되어야 하므로 다음 단계 과제로 남긴다. 이 스크립트의
# Speech_Seq가 있으므로 안건 경계만 찾으면 발언을 안건에 배정할 수 있다.
...
def write_gexf(path: str, nodes: list, edges: list, node_attr_defs: list):
    """의존성 없는 최소 GEXF 1.2 작성기.

    nodes: [{"id":…, "label":…, "attrs":{key:val}, "color":"#rrggbb"|None}]
    edges: [{"source":…, "target":…, "weight":float}]
    node_attr_defs: [(key, "string"|"integer"|"double")]
    Gephi에서 열어 ForceAtlas2 → 시간축 회전(참조 논문 Fig.1 방식)이나
    Timeline(start 속성) 기능을 바로 쓸 수 있다.
    """
    ...
    for i, (key, typ) in enumerate(node_attr_defs):
        L.append(f'<attribute id="{i}" title="{_xesc(key)}" type="{typ}"/>')
    L.append('</attributes><nodes>')
    idx = {key: i for i, (key, _) in enumerate(node_attr_defs)}
    for n in nodes:
        L.append(f'<node id="{_xesc(str(n["id"]))}" label="{_xesc(str(n["label"]))}">')
        avs = []
        for k, v in n.get("attrs", {}).items():
            if k in idx and v is not None:
                avs.append(f'<attvalue for="{idx[k]}" value="{_xesc(str(v))}"/>')
        if avs:
            L.append("<attvalues>" + "".join(avs) + "</attvalues>")
        c = n.get("color")
        if c:
            r, g, b = int(c[1:3], 16), int(c[3:5], 16), int(c[5:7], 16)
            L.append(f'<viz:color r="{r}" g="{g}" b="{b}"/>')
        L.append("</node>")
    L.append("</nodes><edges>")
    for i, e in enumerate(edges):
        L.append(
            f'<edge id="{i}" source="{_xesc(str(e["source"]))}" '
            f'target="{_xesc(str(e["target"]))}" weight="{e.get("weight", 1)}"/>'
        )
...
# ------------------------------------------------------------------------------
# (a) 발언 인접 네트워크: 같은 회의에서 연이어 발언한 발언자 쌍
# ------------------------------------------------------------------------------
_adj = (
    detail
    .filter(pl.col("Speaker") != "")
    .sort(["Title", "Speech_Seq"])
    .with_columns(pl.col("Speaker").shift(1).over("Title").alias("Prev"))
    .filter(pl.col("Prev").is_not_null() & (pl.col("Prev") != pl.col("Speaker")))
    .with_columns([
        # 무방향 정준화: (A,B)와 (B,A)를 같은 간선으로
        pl.when(pl.col("Prev") <= pl.col("Speaker"))
        .then(pl.col("Prev")).otherwise(pl.col("Speaker")).alias("Src"),
        pl.when(pl.col("Prev") <= pl.col("Speaker"))
        .then(pl.col("Speaker")).otherwise(pl.col("Prev")).alias("Dst"),
    ])
    .group_by(["Assembly_Num", "Src", "Dst"])
    .agg(pl.len().alias("Weight"))
    # 1회성 인접(사회자 순서 소음)은 제외: 2회 이상 연이어 말한 쌍만
    .filter(pl.col("Weight") >= 2)
    .sort("Weight", descending=True)
)

_speaker_nodes_df = speaker_keyword_summary.select([
    "Assembly_Num", "Speaker", "Party", "Speaker_Type", "Primary_Role",
    "Speech_Count", "Total_Chars", "First_Speech_Date",
    "Keyword_Total", "Boundary_Per_1k_Chars" if "Boundary_Per_1k_Chars"
    in speaker_keyword_summary.columns else "Keyword_Per_1k_Chars",
])
write_out(_speaker_nodes_df.rename({"Speaker": "Id"}), "speaker_interaction_nodes")
write_out(
    _adj.rename({"Src": "Source", "Dst": "Target"}), "speaker_interaction_edges"
)

_party_colors = {}
def _party_color(p):
    if p not in _party_colors:
        _party_colors[p] = PARTY_PALETTE[len(_party_colors) % len(PARTY_PALETTE)]
    return _party_colors[p]

_gexf_nodes, _seen = [], set()
for r in _speaker_nodes_df.iter_rows(named=True):
    nid = f"{r['Assembly_Num']}|{r['Speaker']}"
    if nid in _seen:
        continue
    _seen.add(nid)
    _gexf_nodes.append({
        "id": nid, "label": r["Speaker"],
        "attrs": {"assembly": r["Assembly_Num"], "party": r["Party"],
                  "type": r["Speaker_Type"], "role": r["Primary_Role"],
                  "speeches": r["Speech_Count"], "chars": r["Total_Chars"],
                  "start": r["First_Speech_Date"]},
        "color": _party_color(r["Party"]),
    })
_gexf_edges = [
    {"source": f"{r['Assembly_Num']}|{r['Src']}",
     "target": f"{r['Assembly_Num']}|{r['Dst']}", "weight": r["Weight"]}
    for r in _adj.iter_rows(named=True)
    if f"{r['Assembly_Num']}|{r['Src']}" in _seen
    and f"{r['Assembly_Num']}|{r['Dst']}" in _seen
]
write_gexf(
    f"{OUTPUT_DIR}/speaker_interaction_network.gexf", _gexf_nodes, _gexf_edges,
    [("assembly", "string"), ("party", "string"), ("type", "string"),
     ("role", "string"), ("speeches", "integer"), ("chars", "integer"),
     ("start", "string")],
)
...
# (b) 회기별 정당 입장 네트워크
# ------------------------------------------------------------------------------
# 이보다 발언량이 적은 (회기,정당)은 입장 벡터가 소음이라 제외.
# 실데이터 기본값 2000자. 환경변수 PSN_MIN_CHARS 로 조절 가능
# (예: 위원회 회의록처럼 회기당 분량이 작은 데이터셋).
_MIN_CHARS = int(os.environ.get("PSN_MIN_CHARS", "2000"))

_stance = (
    detail
    .filter((pl.col("Speaker_Type") == "의원") & pl.col("Session_No").is_not_null())
    .group_by(["Assembly_Num", "Session_No", "Party"])
    .agg(
        [pl.col(en).sum() for en in KEYWORDS_EN]
        + [
            pl.col("Speech_Chars").sum().alias("Total_Chars"),
            ...
            pl.col("Speaker").n_unique().alias("Distinct_Speakers"),
            pl.col("Date").min().alias("Start_Date"),
            pl.col("Discourse_Development").sum().alias("Dev"),
            pl.col("Discourse_Boundary").sum().alias("Bnd"),
        ]
    )
    .filter(pl.col("Total_Chars") >= _MIN_CHARS)
    .with_columns(
        [(pl.col(en) / pl.col("Total_Chars") * 1000).alias(f"{en}_1k")
...
def _cos(u, v):
    ...
    if du == 0 or dv == 0:
        return None       # 키워드 무사용 회기: 유사도 정의 불가 (0과 구분)
    return sum(a * b for a, b in zip(u, v)) / (du * dv)


_snodes, _svec = [], {}
for r in _stance.iter_rows(named=True):
    nid = f"{r['Assembly_Num']}-S{r['Session_No']:03d}|{r['Party']}"
    _svec[nid] = ([r[f"{en}_1k"] for en in KEYWORDS_EN], r)
    _snodes.append({
        "id": nid, "label": f"{r['Party']} (제{r['Session_No']}회)",
        "attrs": {"assembly": r["Assembly_Num"], "session": r["Session_No"],
                  "party": r["Party"], "date": r["Start_Date"],
                  "chars": r["Total_Chars"], "speakers": r["Distinct_Speakers"],
                  "dev": r["Dev"], "bnd": r["Bnd"]},
        "color": _party_color(r["Party"]),
    })

_sedges, _rows_e = [], []
_by_sess, _by_party = defaultdict(list), defaultdict(list)
for nid, (_, r) in _svec.items():
    _by_sess[(r["Assembly_Num"], r["Session_No"])].append(nid)
    _by_party[(r["Assembly_Num"], r["Party"])].append((r["Session_No"], nid))

for _, ids in _by_sess.items():          # 같은 회기 내 정당 간 입장 유사도
    for i in range(len(ids)):
        for j in range(i + 1, len(ids)):
            c = _cos(_svec[ids[i]][0], _svec[ids[j]][0])
            if c is not None:
                _sedges.append({"source": ids[i], "target": ids[j],
                                "weight": round(c, 4)})
                _rows_e.append((ids[i], ids[j], round(c, 4), "within_session"))
for _, lst in _by_party.items():         # 같은 정당의 인접 회기 연결(시간 등뼈)
    lst.sort()
    for (s1, n1), (s2, n2) in zip(lst, lst[1:]):
        c = _cos(_svec[n1][0], _svec[n2][0])
        _sedges.append({"source": n1, "target": n2,
                        "weight": round(c, 4) if c is not None else 0.0})
        _rows_e.append((n1, n2, round(c, 4) if c is not None else None,
                        "temporal"))

write_out(
    _stance.with_columns(
        pl.concat_str([
            pl.col("Assembly_Num"), pl.lit("-S"),
            pl.col("Session_No").cast(pl.Utf8).str.zfill(3),
            pl.lit("|"), pl.col("Party"),
        ]).alias("Id")
    ),
    "party_stance_nodes",
...
write_out(
    pl.DataFrame(_rows_e, schema=["Source", "Target", "Weight", "Edge_Type"],
                 orient="row"),
    "party_stance_edges",
)
write_gexf(
    f"{OUTPUT_DIR}/party_stance_network.gexf", _snodes, _sedges,
    [("assembly", "string"), ("session", "integer"), ("party", "string"),
     ("date", "string"), ("chars", "integer"), ("speakers", "integer"),
     ("dev", "integer"), ("bnd", "integer")],
)

# ------------------------------------------------------------------------------
# (c) 자체완결 SVG 타임라인 — Bingenheimer Fig.1의 국회판
# ------------------------------------------------------------------------------
# x = 회기 시작일, y = 정당 레인, 원 크기 = 발언 분량, 색 = 정당,
# 세로 회색선 = 같은 회기 정당 간 입장 유사도(굵을수록 유사),
# 가로선 = 같은 정당의 회기 간 연속성. 외부 라이브러리·JS 없이 열린다.
...
def build_overview_html(path: str):
    pts = []
    for nid, (_, r) in _svec.items():
        d = _iso(r["Start_Date"])
        if d:
    ...
    # 발언량 상위 정당에 레인 배정, 나머지는 '기타' 레인으로
    vol = Counter()
    for _, _, r in pts:
        vol[r["Party"]] += r["Total_Chars"]
    lanes = [p for p, _ in vol.most_common(12)]
    lane_of = {p: i for i, p in enumerate(lanes)}
    n_lanes = len(lanes) + (1 if len(vol) > len(lanes) else 0)

    W, H, ML, MR, MT, MB = 1500, 120 + 46 * max(n_lanes, 1), 150, 40, 70, 60
    def X(d): return ML + (d - dmin).days / span * (W - ML - MR)
    def Y(p): return MT + lane_of.get(p, len(lanes)) * 46 + 23
    cmax = max(r["Total_Chars"] for _, _, r in pts)
    def R(c): return max(2.5, min(15.0, 15.0 * (c / cmax) ** 0.5))

    pos = {nid: (X(d), Y(r["Party"])) for nid, d, r in pts}
    S = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" '
         f'style="background:#111;font-family:sans-serif">']
    S.append(f'<text x="{ML}" y="34" fill="#eee" font-size="20" font-weight="bold">'
             f'대한민국 국회 담론의 역사 — 회기별 정당 입장 네트워크</text>')
    S.append(f'<text x="{ML}" y="54" fill="#888" font-size="12">'
             f'원 크기=발언 분량 · 색=정당 · 세로선=같은 회기 정당 간 키워드 프로필 '
             f'유사도 · 가로선=정당의 회기 간 연속</text>')

    # 대수 경계 세로 점선
    firsts = {}
    for nid, d, r in pts:
        a = r["Assembly_Num"]
        if a not in firsts or d < firsts[a]:
            firsts[a] = d
    for a, d in sorted(firsts.items(), key=lambda kv: kv[1]):
        x = X(d)
        S.append(f'<line x1="{x:.1f}" y1="{MT-8}" x2="{x:.1f}" y2="{H-MB}" '
                 f'stroke="#444" stroke-dasharray="4,4"/>')
        S.append(f'<text x="{x+4:.1f}" y="{MT-12}" fill="#aaa" font-size="12">{a} '
                 f'({d.year})</text>')

    # 정당 레인 라벨
    for p in lanes:
        S.append(f'<text x="10" y="{Y(p)+4:.1f}" fill="{_party_color(p)}" '
                 f'font-size="12">{_xesc(p[:12])}</text>')
    if n_lanes > len(lanes):
        S.append(f'<text x="10" y="{MT + len(lanes)*46 + 27}" fill="#999" '
                 f'font-size="12">기타</text>')

    # 간선 (노드보다 먼저 그려서 아래 깔리게)
    for e in _sedges:
        a, b = pos.get(e["source"]), pos.get(e["target"])
        if not a or not b:
            continue
        w = e["weight"] or 0
        temporal = abs(a[1] - b[1]) < 1e-6
        if temporal:
            S.append(f'<line x1="{a[0]:.1f}" y1="{a[1]:.1f}" x2="{b[0]:.1f}" '
                     f'y2="{b[1]:.1f}" stroke="#666" stroke-width="0.8" '
                     f'opacity="{0.15 + 0.35*max(w,0):.2f}"/>')
        elif w >= 0.4:
            S.append(f'<line x1="{a[0]:.1f}" y1="{a[1]:.1f}" x2="{b[0]:.1f}" '
                     f'y2="{b[1]:.1f}" stroke="#8ab" '
                     f'stroke-width="{0.4 + 1.6*w:.2f}" opacity="0.25"/>')

    # 노드
    for nid, d, r in pts:
        x, y = pos[nid]
        tip = (f"{r['Assembly_Num']} 제{r['Session_No']}회 · {r['Party']} · "
               f"{r['Start_Date']} · {r['Total_Chars']:,}자 · "
               f"발전 {r['Dev']} / 경계 {r['Bnd']}")
        S.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{R(r["Total_Chars"]):.1f}" '
                 f'fill="{_party_color(r["Party"])}" fill-opacity="0.85" '
                 f'stroke="#000" stroke-width="0.5"><title>{_xesc(tip)}</title>'
                 f'</circle>')
    ...
    for yr in range(dmin.year, dmax.year + 1):
        d = _date(yr, 1, 1)
        if dmin <= d <= dmax:
            S.append(f'<text x="{X(d):.1f}" y="{H-MB+24}" fill="#777" '
                     f'font-size="11" text-anchor="middle">{yr}</text>')
    S.append("</svg>")

    html = ("<!doctype html><meta charset='utf-8'>"
            "<title>국회 담론 타임라인</title>"
            "<body style='margin:0;background:#111'>" + "".join(S) + "</body>")
    with open(path, "w", encoding="utf-8") as f:
        f.write(html)
    return True


_overview_ok = build_overview_html(f"{OUTPUT_DIR}/assembly_history_overview.html")
...
_residual = int(final_collected["Residual_Header_Flag"].sum())
print(f"본문에 헤더가 남은 것으로 의심되는 행(Residual_Header_Flag): {_residual:,} "
      f"({_residual / total_rows * 100:.2f}%)")
print("  -> 이 행들도 검수 파일에 포함됨. 반복 출현 직책은 TITLE_CORE에, "
      "이름은 input/officer_names.csv 에 추가해 환류할 것")
...
print(f"  {'speaker_interaction_network.gexf':<34} 노드 {len(_gexf_nodes):,} · 간선 {len(_gexf_edges):,}")
print(f"  {'party_stance_network.gexf':<34} 노드 {len(_snodes):,} · 간선 {len(_sedges):,}")
if _overview_ok:
    print(f"  {'assembly_history_overview.html':<34} 타임라인 시각화 (브라우저로 열기)")
